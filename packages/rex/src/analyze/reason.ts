...
/**
 * First-line markers written by `ndx init` at the top of generated assistant
 * instruction files (CLAUDE.md, AGENTS.md). When a doc file begins with one
 * of these, its contents describe the n-dx tooling rather than the user's
 * project and would contaminate analyze/plan prompts.
 */
const NDX_GENERATED_MARKERS = [
  "<!-- Generated by ndx init",
  "# Generated by ndx init",
];

function isNdxGenerated(content: string): boolean {
  const firstLine = content.trimStart().split("\n", 1)[0] ?? "";
  return NDX_GENERATED_MARKERS.some((m) => firstLine.startsWith(m));
}
 ...
 * useful is found. Files generated by `ndx init` are skipped so the
 * tool's own docs don't leak into LLM prompts as "project context".
 */
export async function readProjectContext(dir: string): Promise<string> {
  ...
  for (const name of PROJECT_DOC_FILES) {
    ...
    try {
      ...
      if (isNdxGenerated(content)) continue;
