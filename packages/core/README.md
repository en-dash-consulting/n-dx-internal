...
| | | |
|:---:|:---:|:---:|
| [![SourceVision](../sourcevision/SourceVision.png)](../sourcevision) | [![Rex](../rex/Rex.png)](../rex) | [![Hench](../hench/Hench.png)](../hench) |
| **[SourceVision](../sourcevision)** | **[Rex](../rex)** | **[Hench](../hench)** |
...
| Package | Description |
|---------|-------------|
| **[@n-dx/sourcevision](../sourcevision)** | Static analysis: file inventory, import graph, zone detection (Louvain), React component catalog. Produces `.sourcevision/CONTEXT.md` and `llms.txt`. |
| **[@n-dx/rex](../rex)** | PRD management: hierarchical epics/features/tasks/subtasks, LLM-powered analysis and recommendations. Stores state in `.rex/prd_tree/` (slug-based folder tree). |
| **[@n-dx/hench](../hench)** | Autonomous agent: picks rex tasks, builds briefs, runs LLM tool-use loops with security guardrails. Records runs in `.hench/runs/`. |
| **[@n-dx/llm-client](../llm-client)** | Vendor-neutral LLM foundation: Claude and Codex adapters, provider registry, token usage tracking. |
| **[@n-dx/web](../web)** | Dashboard and unified MCP HTTP server: browser-based project dashboard with zone maps and PRD status. |
...
> **Legacy PRD migration.** If upgrading from a previous layout using `.rex/prd.md` (flat Markdown) or `.rex/prd.json`, run `rex migrate-to-folder-tree` to convert to the folder tree format. The command optionally deletes the legacy source file after migration. See the [rex README](../rex) for details.
...
See the [hench README](../hench#security) for the full configuration reference.
