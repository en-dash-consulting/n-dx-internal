...
# Hold first Ctrl+C during rollback prompt and require second Ctrl+C to exit

🟠 [completed]

## Summary

When a hench run loop is interrupted with Ctrl+C, the existing flow opens a 'Revert n uncommitted file(s)?' rollback prompt. Currently a second Ctrl+C while that prompt is open can drop the user into an inconsistent state. Make the prompt absorb the first interrupt signal so the user can deliberate, and only exit the process when a second Ctrl+C is received while the prompt is still open. The first Ctrl+C should display a hint that pressing Ctrl+C again will abort the rollback prompt and exit.

## Info

- **Status:** completed
- **Priority:** high
- **Tags:** hench, cli, run-loop, interrupt
- **Level:** task
- **Started:** 2026-04-29T15:43:32.822Z
- **Completed:** 2026-04-29T15:50:48.822Z
- **Duration:** 7m
