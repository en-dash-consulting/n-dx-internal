...
Extend the tree command output with the specified color scheme: completed items in purple, in-progress items in yellow with `**title**` surrounding asterisks, and pending items with no color. Must reuse the shared ANSI color utility already used across other CLI commands and must respect `NO_COLOR` and non-TTY detection.
