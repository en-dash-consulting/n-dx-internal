...
Trace every code path reachable from ndx add, rex add, and the smart-add LLM pipeline. Remove any remaining prd.md read fallbacks and any write calls that touch .rex/prd.md. The folder-tree must be the sole source of truth at runtime; legacy migration remains the only surface that reads prd.md.
