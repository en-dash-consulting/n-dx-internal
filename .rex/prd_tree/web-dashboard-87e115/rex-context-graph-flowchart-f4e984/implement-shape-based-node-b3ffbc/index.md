...
# Implement shape-based node classification and rendering for Rex context graph nodes

🟠 [pending]

## Summary

Classify each graph node by inspecting its position in the `.rex/prd_tree/` structure and assign a shape according to these rules: diamond — parent node that contains both an index.md and at least one other non-index.md file; square — parent node with only non-index.md files and no subdirectory children; trapezoid — parent node whose children are exclusively subdirectories (no direct files); triangle — leaf task node with no children of any kind; circle — default for nodes that don't match any rule above. Update the SVG/graph rendering layer to draw each node in its assigned shape and add a shape legend to the graph UI.
