...
# Wire click-through on context graph nodes to open the existing Rex task detail panel

🟠 [pending]

## Summary

Make each node in the Rex context graph clickable. On click, open the existing Rex task detail panel (already implemented in the tasks view) displaying the selected item's full information — title, description, status, acceptance criteria, tags, LoE, and branch/commit attribution. The panel must be the same component used in the tasks view, not a new implementation. Clicking the same node again or pressing Escape should close the panel.

## Info

- **Status:** pending
- **Priority:** high
- **Tags:** ui, graph, rex, accessibility
- **Level:** task
