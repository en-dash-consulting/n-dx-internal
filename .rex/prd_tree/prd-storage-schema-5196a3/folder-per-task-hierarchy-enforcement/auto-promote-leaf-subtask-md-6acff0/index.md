...
# Auto-promote leaf subtask .md files to folder-with-index.md when children are added

🟠 [pending]

## Summary

When any write path attempts to add a child under a subtask that currently exists as a leaf .md file, atomically convert the .md file into a folder of the same title slug, move its content into a new index.md inside that folder, then write the new child alongside it. This must happen transparently for rex CLI, MCP add_item, ndx plan acceptance, and any other code path that creates children. Use atomic rename/write semantics so a crash mid-promotion leaves either the old .md file or the new folder, never both.

## Info

- **Status:** pending
- **Priority:** high
- **Tags:** prd-storage, migration, serializer
- **Level:** task
