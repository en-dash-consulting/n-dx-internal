...
const ALLOWED = new Set([
  ...
  // Google OAuth — opens the system browser via execFile for the auth flow
  "packages/core/google-auth.js",
...
const BOUNDARY_FILES = [
  ...
  },
  {
    ...
    maxExports: 145,
    description: "hench→llm-client gateway (config, constants, JSON, output, errors, exec, runtime-contract, codex-policy, diagnostics, tool-schema, provider-registry, vendor-error-classification, failover, color/model helpers, token-accumulation, google/tier model catalogs — TIER_MODELS + GOOGLE_MODELS added for the Google vendor integration; Gemini tool-loop surface — toGeminiFunctionDeclaration(s), GeminiFunctionDeclaration/GeminiSchema and GeminiToolProvider/GeminiContent/GeminiPart/GeminiToolBlock/GeminiGenerateResult/GenerateContentWithToolsArgs added for the Gemini agentic tool-use loop; Google OAuth detection — detectGoogleAuthMethod + GoogleAuthResult added for auth method display in vendor header; error code registry — E_TIMEOUT/E_MALFORMED_RESPONSE/E_NULL_RESPONSE/E_AUTH_FAILURE/E_NETWORK_ERROR/E_UNKNOWN/mapFailureCategoryToErrorEntry added for structured token-exhaustion error handling; rate-limit helpers — extractRefreshAt + formatRetryCountdown added for token-quota wait-and-retry)",
  },
];
