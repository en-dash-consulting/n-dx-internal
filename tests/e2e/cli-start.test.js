...
describe("n-dx start", () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), "ndx-start-e2e-"));
  ...
  describe("unknown subcommand", () => {
    it("exits 1 for unknown start subcommand", () => {
      const { stderr, code } = runResult(["restart", tmpDir]);
      expect(code).toBe(1);
      expect(stderr).toContain("Unknown start subcommand");
    });
  });
  ...
  // ── Messaging uses "server" label ───────────────────────────────────

  describe("messaging", () => {
    it("uses 'n-dx server' in background output", async () => {
      const port = await findAvailablePort();
      const { stdout, code } = runResult([`--port=${port}`, "--background", tmpDir]);
      expect(code).toBe(0);
      expect(stdout).toContain("n-dx server");
  ...
  describe("help text", () => {
    it("shows start in the main help output", () => {
      ...
      expect(output).toContain("dashboard + MCP");
    });
  });
});
