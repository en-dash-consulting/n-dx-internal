# Agent Engine — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Agent Engine (`agent-engine`)
Files: 109, Cohesion: 0.99, Coupling: 0.01
Description: Autonomous agent execution engine handling task analysis, lifecycle management, and tool dispatch with adaptive workflow capabilities.
Entry points: packages/hench/src/process/index.ts, packages/hench/src/tools/rex.ts
Lines: 21910
...
Outgoing (this zone → other zones):
  → completion-validation: packages/hench/src/agent/index.ts → packages/hench/src/validation/completion.ts; packages/hench/src/agent/index.ts → packages/hench/src/validation/completion.ts; packages/hench/src/agent/lifecycle/cli-loop.ts → packages/hench/src/validation/completion.ts; packages/hench/src/public.ts → packages/hench/src/validation/completion.ts; packages/hench/src/tools/rex.ts → packages/hench/src/validation/completion.ts

Incoming (other zones → this zone):
  ← completion-validation: packages/hench/src/validation/completion.ts → packages/hench/src/process/index.ts; packages/hench/tests/unit/tools/completion-validation.test.ts → packages/hench/src/tools/rex.ts
...
[observation] [info] Bidirectional coupling with hench-2 zone suggests potential architectural refinement opportunity
[observation] [info] Sophisticated agent capabilities with adaptive analysis and workflow management
[observation] [info] Strong domain organization with clear separation between analysis, lifecycle, and tool concerns
...
- Largest zone (109 files) with excellent cohesion (0.99) indicates well-organized complex domain logic
- Comprehensive agent analysis modules (adaptive.ts, review.ts, stuck.ts) suggest sophisticated task handling
- Multiple entry points (process/index.ts, tools/rex.ts) provide flexible integration options
- Strong domain organization with clear separation between analysis, lifecycle, and tool concerns
- Sophisticated agent capabilities with adaptive analysis and workflow management
- Bidirectional coupling with hench-2 zone suggests potential architectural refinement opportunity
- [call graph] 2040 internal calls, 5 outgoing, 13 incoming (cohesion: 1, coupling: 0)
...
- **Tests** (`hench/tests`): 73 files, cohesion 0.91, coupling 0.09
  - Has 3 nested sub-zone(s)
- **Tools** (`hench/tools`): 11 files, cohesion 0.79, coupling 0.21
- **Cli** (`hench/cli`): 14 files, cohesion 0.67, coupling 0.33
- **Guard** (`hench/guard`): 10 files, cohesion 0.84, coupling 0.16
