# Web Gateway Services — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Web Gateway Services (`web-domain-gateways`)
Files: 56, Cohesion: 0.99, Coupling: 0.01
Description: Owns the server entrypoints, domain gateways, and schemas that expose backend data over CLI/public interfaces.
...
packages/web/src/server/routes-sourcevision.ts (TypeScript, 615 lines, source)
...
packages/web/tests/integration/pr-markdown-refresh.test.ts (TypeScript, 159 lines, test)
...
packages/web/tests/unit/server/routes-sourcevision.test.ts (TypeScript, 362 lines, test)
...
Outgoing (this zone → other zones):
  ...
  → web-viewer-shell: packages/web/tests/integration/pr-markdown-refresh.test.ts → packages/web/src/viewer/views/pr-markdown.ts

Incoming (other zones → this zone):
  ← web-viewer-shell: packages/web/src/viewer/loader.ts → packages/web/src/schema/data-files.ts; packages/web/src/viewer/main.ts → packages/web/src/schema/data-files.ts
...
[observation] [info] Gateway modules (domain, rex, MCP) overlap; clarifying their contracts would keep integrations pluggable as new backends appear.
[observation] [info] Treat `schema/data-files.ts` as the canonical published schema to prevent drift across downstream clients.
[observation] [info] `server/index.ts` should standardize logging/error hooks before binding ports to avoid divergent diagnostics across deployments.
...
- Define clearer boundaries between `server/domain-gateway.ts`, `rex-gateway.ts`, and `mcp-deps.ts` so each ingress path stays pluggable without hidden coupling.
- Instrument `packages/web/src/server/index.ts` to enforce consistent logging and error handling before binding ports configured via `server/port.ts`.
- Promote `schema/data-files.ts` as the single public schema entrypoint so downstream clients stay version-aligned.
- Gateway modules (domain, rex, MCP) overlap; clarifying their contracts would keep integrations pluggable as new backends appear.
- `server/index.ts` should standardize logging/error hooks before binding ports to avoid divergent diagnostics across deployments.
- Treat `schema/data-files.ts` as the canonical published schema to prevent drift across downstream clients.
- [call graph] 1044 internal calls, 21 outgoing, 1 incoming (cohesion: 0.98, coupling: 0.02)
