# Hench Agent Orchestration — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Hench Agent Orchestration (`hench-agent-core`)
Files: 97, Cohesion: 0.96, Coupling: 0.04
Description: Hosts the core Hench agent workflows, analyses, guardrails, and schemas that drive autonomous operation.
Entry points: packages/hench/src/guard/index.ts, packages/hench/src/schema/v1.ts, packages/hench/src/tools/rex.ts
Lines: 20749
...
packages/hench/src/guard/paths.ts (TypeScript, 75 lines, source)
...
Outgoing (this zone → other zones):
  → hench-tooling-process: packages/hench/src/agent/analysis/review.ts → packages/hench/src/process/index.ts; packages/hench/src/agent/lifecycle/cli-loop.ts → packages/hench/src/tools/index.ts; packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/tools/contracts.ts; packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/tools/dispatch.ts; packages/hench/src/agent/lifecycle/shared.ts → packages/hench/src/process/index.ts; packages/hench/src/agent/lifecycle/shared.ts → packages/hench/src/tools/index.ts; packages/hench/src/guard/paths.ts → packages/hench/src/shared/glob.ts; packages/hench/src/tools/rex.ts → packages/hench/src/process/index.ts; packages/hench/src/validation/completion.ts → packages/hench/src/process/index.ts; packages/hench/tests/unit/agent/task-failure.test.ts → packages/hench/src/agent/tools.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/tools/shell.ts

Incoming (other zones → this zone):
  ← hench-tooling-process: packages/hench/src/tools/dispatch.ts → packages/hench/src/tools/rex.ts; packages/hench/src/tools/index.ts → packages/hench/src/tools/rex.ts; packages/hench/src/tools/index.ts → packages/hench/src/tools/rex.ts; packages/hench/tests/unit/tools/files.test.ts → packages/hench/src/guard/index.ts; packages/hench/tests/unit/tools/files.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/git.test.ts → packages/hench/src/guard/index.ts; packages/hench/tests/unit/tools/git.test.ts → packages/hench/src/schema/v1.ts
...
[relationship] [warning] Layered-but-leaky relationship with hench-tooling-process: heavy downstream invocation combined with reverse callbacks keeps the dependency cycle alive.
...
- Multiple entrypoints (`agent/index.ts`, guards, schema) indicate this zone is the public API for higher layers.
- Imports into and from `hench-2` show a dependency cycle that needs careful boundary management.
- Core analyses (`adaptive`, `review`, `stuck`, `summary`) and lifecycle loops live together; document which modules form the stable API so downstream tools call only supported functions.
- Despite high cohesion, there are 11 imports into `hench-2`, so consider promoting frequently re-used helpers (e.g., lifecycle contracts) into a shared `hench-common` layer to reduce back-and-forth dependencies.
- Entry points such as `schema/v1.ts` expose versioned contracts; enforce semantic versioning tests when updating schema to prevent breakage in tool adapters.
- The agent core drives 56 calls into hench-tooling-process while still handling six callbacks from it, so the boundary is logically layered but still leaky, suggesting event or message interfaces would better enforce one-way control flow.
- Layered-but-leaky relationship with hench-tooling-process: heavy downstream invocation combined with reverse callbacks keeps the dependency cycle alive.
- [call graph] 1931 internal calls, 56 outgoing, 6 incoming (cohesion: 0.97, coupling: 0.03)
...
- **Tests** (`hench/tests`): 57 files, cohesion 0.85, coupling 0.15
  - Has 2 nested sub-zone(s)
- **Tests 2** (`hench/tests-2`): 9 files, cohesion 0.75, coupling 0.25
