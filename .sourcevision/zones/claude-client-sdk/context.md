# Anthropic Client SDK — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Anthropic Client SDK (`claude-client-sdk`)
Files: 28, Cohesion: 1.00, Coupling: 0.00
Description: Implements the CLI/API client for Claude, spanning authentication, JSON handling, and provider wiring for downstream packages.
Lines: 4832
...
[observation] [info] Add contract or smoke tests near `src/cli-provider.ts` to ensure CLI-specific defaults stay aligned with Anthropic API changes.
[observation] [info] Since cohesion is perfect, consider extracting lightweight docs highlighting how `auth.ts`, `config.ts`, and `exec.ts` compose for easy reuse.
[observation] [info] Surface the provider boundary in `src/create-client.ts` as a documented interface so other zones can mock it without re-reading internals.
...
- Implements full provider stack (auth, config, exec) making it reusable by other zones.
- Utility focus suggests good candidate for publishing as standalone package.
- Surface the provider boundary in `src/create-client.ts` as a documented interface so other zones can mock it without re-reading internals.
- Add contract or smoke tests near `src/cli-provider.ts` to ensure CLI-specific defaults stay aligned with Anthropic API changes.
- Since cohesion is perfect, consider extracting lightweight docs highlighting how `auth.ts`, `config.ts`, and `exec.ts` compose for easy reuse.
