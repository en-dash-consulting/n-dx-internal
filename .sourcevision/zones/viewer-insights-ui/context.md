# Viewer Insights UI — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Viewer Insights UI (`viewer-insights-ui`)
Files: 137, Cohesion: 1.00, Coupling: 0.00
Description: Schema-aware viewer components, validators, and panels that render adaptive task data to the web client.
Lines: 48246
...
Outgoing (this zone → other zones):
  → adaptive-web-gateway: packages/web/src/viewer/loader.ts → packages/web/src/schema/data-files.ts; packages/web/src/viewer/main.ts → packages/web/src/schema/data-files.ts
...
[observation] [info] Add schema-driven Storybook fixtures or tests so components consuming `schema/v1.ts` catch breaking contract updates before deployment.
[observation] [info] Bundle viewer components around shared `constants.ts` and typed props to avoid duplication as panels like `active-tasks-panel.ts` and `breadcrumb.ts` expand.
[observation] [info] Perfect cohesion/coupling (1/0) indicates the UI layer is a pure consumer library; keep backend awareness gated through shared schema modules only.
...
- Centralize schema usage through `packages/web/src/schema/v1.ts` and `packages/web/src/schema/validate.ts` so UI components update safely with backend changes.
- Prioritize component cohesion inside `packages/web/src/viewer/components/*` by sharing constants/types instead of duplicating props.
- Establish lightweight mocks for `viewer/components/data-display` to keep visual regression coverage high without backend dependencies.
- Perfect cohesion/coupling (1/0) indicates the UI layer is a pure consumer library; keep backend awareness gated through shared schema modules only.
- Bundle viewer components around shared `constants.ts` and typed props to avoid duplication as panels like `active-tasks-panel.ts` and `breadcrumb.ts` expand.
- Add schema-driven Storybook fixtures or tests so components consuming `schema/v1.ts` catch breaking contract updates before deployment.
