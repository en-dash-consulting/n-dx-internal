...
Description: Implements CLI-friendly client creation, auth, and provider plumbing for Anthropic Claude endpoints.
Lines: 4832
...
[pattern] [info] Plug-in provider pattern: isolation from other zones means the Claude client can be swapped or versioned independently, but requires cross-zone contract checks to guarantee parity with shared clients.
...
- Mirrors the structure of the general `llm-client`, suggesting opportunities for shared abstractions.
- Contains multiple provider/front-end files that should stay aligned through common helper exports.
- Auth, CLI provider, and API provider layers all live here; ensure they continue to share the same `config.ts` schema so flags like model selection stay consistent across entrypoints.
- Utility modules (`exec.ts`, `help-format.ts`, `json.ts`) are unique to Claude today—extract shared helpers into a neutral package if you plan to support additional Claude-facing tools.
- With perfect cohesion/coupling metrics, guard against accidental imports from Hench or global tooling by exposing only factory functions rather than raw HTTP helpers.
- With 317 internal calls and zero cross-zone interactions, this package can be treated as a plug-in provider that can ship independently, but it currently lacks any explicit contract tests tying it back to the shared LLM client surface.
- Plug-in provider pattern: isolation from other zones means the Claude client can be swapped or versioned independently, but requires cross-zone contract checks to guarantee parity with shared clients.
