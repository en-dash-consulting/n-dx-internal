# Adaptive Web Gateway — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Adaptive Web Gateway (`adaptive-web-gateway`)
Files: 53, Cohesion: 0.99, Coupling: 0.01
Description: Runs the HTTP server, adaptive routing, and MCP-facing gateways that serve schema-backed data to clients.
...
Incoming (other zones → this zone):
  ← viewer-schema-surface: packages/web/src/viewer/loader.ts → packages/web/src/schema/data-files.ts; packages/web/src/viewer/main.ts → packages/web/src/schema/data-files.ts
...
[observation] [info] Add runtime health checks in `server/index.ts` or `routes-adaptive.ts` to surface MCP dependency failures before they cascade to clients.
[observation] [info] Factor the logic in `server/domain-gateway.ts` into explicit integration boundaries to make external service mocks simpler for tests.
[observation] [info] Keep exporting shared dataset metadata through `schema/data-files.ts` so both the CLI and server stay in lockstep on file contracts.
...
- Routes, gateways, and MCP adapters converge here.
- Near-perfect cohesion suggests a clean layering opportunity for contracts.
- Keep exporting shared dataset metadata through `schema/data-files.ts` so both the CLI and server stay in lockstep on file contracts.
- Factor the logic in `server/domain-gateway.ts` into explicit integration boundaries to make external service mocks simpler for tests.
- Add runtime health checks in `server/index.ts` or `routes-adaptive.ts` to surface MCP dependency failures before they cascade to clients.
