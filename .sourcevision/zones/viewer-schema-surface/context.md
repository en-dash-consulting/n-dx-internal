# Viewer & Schema Experience — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Viewer & Schema Experience (`viewer-schema-surface`)
Files: 137, Cohesion: 1.00, Coupling: 0.00
Description: Owns the v1 schema definitions, validation utilities, and the full viewer component library that renders those datasets in the UI.
Lines: 48246
...
[observation] [info] Enforce stricter type exports from `schema/v1.ts` and `schema/validate.ts` so the 137-file component tree stays aligned with data contracts.
[observation] [info] Keep cross-component primitives under `viewer/components/**` to avoid duplicated styling and UX logic across panels like `active-tasks-panel.ts` and `breadcrumb.ts`.
[observation] [info] Since two imports point into `web`, define a minimal interface module for those needs to keep the viewer’s dependency on the server zone narrow.
...
- Bridges schema contracts and the UI that consumes them.
- Depends lightly on the server zone for data loading hooks.
- Enforce stricter type exports from `schema/v1.ts` and `schema/validate.ts` so the 137-file component tree stays aligned with data contracts.
- Keep cross-component primitives under `viewer/components/**` to avoid duplicated styling and UX logic across panels like `active-tasks-panel.ts` and `breadcrumb.ts`.
- Since two imports point into `web`, define a minimal interface module for those needs to keep the viewer’s dependency on the server zone narrow.
