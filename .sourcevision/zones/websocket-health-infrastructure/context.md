# WebSocket Health Infrastructure — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: WebSocket Health Infrastructure (`websocket-health-infrastructure`)
Files: 6, Cohesion: 0.50, Coupling: 0.50
Risk: healthy (score: 0.50)
Description: WebSocket server implementation and connection health tracking services, keeping real-time transport concerns isolated from the broader HTTP server layer.
...
Incoming (other zones → this zone):
  ← web-dashboard-application: packages/web/src/public.ts → packages/web/src/server/websocket.ts; packages/web/src/server/index.ts → packages/web/src/server/websocket.ts; packages/web/src/server/routes-hench.ts → packages/web/src/server/websocket.ts; packages/web/src/server/routes-rex.ts → packages/web/src/server/websocket.ts; packages/web/src/server/start.ts → packages/web/src/server/websocket.ts; packages/web/src/server/start.ts → packages/web/src/server/ws-health-tracker.ts

</imports>

<findings>

[observation] [warning] Cohesion of 0.5 is at the low boundary — if the WebSocket transport and health tracker diverge in responsibilities over time, splitting them into separate zones would improve clarity.
[observation] [info] Coupling of 0.5 with 6 imports from web-dashboard-application means changes here can ripple broadly; a stable interface contract (e.g., an explicit export surface) would reduce breakage risk.
[observation] [info] Four test files for two source files indicates thorough coverage of the real-time transport layer, which is a reliability-sensitive component.
[relationship] [info] Spoke isolation is intact: the WebSocket layer does not import from other satellite zones. Given its high coupling (0.5) to web-dashboard-application, any future extraction of the WebSocket interface should target the hub boundary, not lateral zone relationships.
[anti-pattern] [warning] boundary-check.test.ts is likely a misplaced file: its name implies cross-zone or package-boundary testing rather than WebSocket infrastructure behavior. If it imports from zones outside websocket-health-infrastructure, it belongs in an integration or e2e test directory, not collocated with the WebSocket unit tests.
[suggestion] [info] Rename zone from 'websocket-health-infrastructure' to 'websocket-transport' or 'realtime-transport-layer' to reflect that the WebSocket transport is the primary concern and health tracking is a secondary overlay. Current name causes the zone to be mis-scoped in any zone-listing context.
[suggestion] [warning] Zone satisfies both fragile-zone conditions simultaneously (cohesion 0.5, coupling 0.5) — the only such zone in the codebase. Treat as a warning-priority refactor target: either split websocket.ts (transport) from ws-health-tracker.ts (monitoring) into separate zones, or document an explicit stability contract for the combined surface to prevent further cohesion degradation.
