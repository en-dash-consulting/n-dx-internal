# Anthropic Provider Adapter — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Anthropic Provider Adapter (`claude-client`)
Files: 28, Cohesion: 1.00, Coupling: 0.00
Description: Implements the Anthropic-specific API, CLI wiring, authentication helpers, and JSON utilities that feed the surrounding tooling.
Lines: 4832
...
[observation] [info] Automate regression tests for `api-provider.ts` against recorded Anthropic responses to detect breaking API changes quickly.
[observation] [info] Harden `auth.ts` with validation and logging around credential loading so misconfigured Anthropic keys fail fast.
[observation] [info] Refactor repeated utilities (`exec.ts`, `help-format.ts`, `json.ts`) into a shared module with `llm-client` to ease cross-provider maintenance.
...
- Create a shared base client with `llm-client` to remove duplicated helpers like `exec.ts` and `help-format.ts`.
- Codify env-var and key management in `auth.ts` to prevent integration mistakes when rotating credentials.
- Add contract tests for `api-provider.ts` to lock down expected prompts and response parsing for Claude upgrades.
- Refactor repeated utilities (`exec.ts`, `help-format.ts`, `json.ts`) into a shared module with `llm-client` to ease cross-provider maintenance.
- Harden `auth.ts` with validation and logging around credential loading so misconfigured Anthropic keys fail fast.
- Automate regression tests for `api-provider.ts` against recorded Anthropic responses to detect breaking API changes quickly.
