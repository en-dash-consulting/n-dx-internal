# Web Infrastructure — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Web Infrastructure (`web-infrastructure`)
Files: 59, Cohesion: 0.98, Coupling: 0.02
Description: Core web platform infrastructure providing CLI entry points, public APIs, and foundational schema definitions.
...
Incoming (other zones → this zone):
  ← viewer-platform: packages/web/src/viewer/loader.ts → packages/web/src/schema/data-files.ts; packages/web/src/viewer/main.ts → packages/web/src/schema/data-files.ts; packages/web/tests/integration/pr-markdown-refresh.test.ts → packages/web/src/server/routes-sourcevision.ts; packages/web/tests/integration/pr-markdown-refresh.test.ts → packages/web/src/server/types.ts
...
[observation] [info] Strong architectural foundation with excellent cohesion (0.98) and minimal coupling (0.02), properly isolating core web infrastructure concerns
...
- High cohesion indicates well-defined core responsibilities without feature sprawl
- Minimal coupling suggests clean separation between infrastructure and application logic
- Central position in web package import graph makes it a stable foundation for higher-level web components
- Strong architectural foundation with excellent cohesion (0.98) and minimal coupling (0.02), properly isolating core web infrastructure concerns
