# Hench Agent Runtime — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Hench Agent Runtime (`hench-core-agent`)
Files: 100, Cohesion: 0.97, Coupling: 0.03
Description: Contains the agent brain—analysis workflows, lifecycle management, schema definitions, and process entrypoints powering task execution.
...
Outgoing (this zone → other zones):
  ...
  → hench-guard-rails: packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/guard/index.ts; packages/hench/src/schema/v1.ts → packages/hench/src/guard/contracts.ts; packages/hench/src/tools/files.ts → packages/hench/src/guard/paths.ts
...
Incoming (other zones → this zone):
  ...
  ← hench-guard-rails: packages/hench/tests/unit/guard/guard-integration.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/tools/shell.ts
...
[observation] [info] Document the multiple entrypoints (process/schema/tools) so operators understand which binary to invoke for automation, reducing misconfiguration.
[observation] [info] Given cross-zone dependencies from hench-2/3, publish explicit extension points in `src/agent/index.ts` to keep integrations decoupled.
[observation] [info] Maintain the clean layering by keeping new features inside `agent/analysis` or `agent/lifecycle` rather than leaking into tool adapters.
...
- Entry points span process, schema, and tool adapters, reflecting a rich runtime core.
- Maintains near-perfect cohesion, showing tight alignment of responsibilities.
- Maintain the clean layering by keeping new features inside `agent/analysis` or `agent/lifecycle` rather than leaking into tool adapters.
- Given cross-zone dependencies from hench-2/3, publish explicit extension points in `src/agent/index.ts` to keep integrations decoupled.
- Document the multiple entrypoints (process/schema/tools) so operators understand which binary to invoke for automation, reducing misconfiguration.
...
- **Tests** (`hench/tests`): 67 files, cohesion 0.89, coupling 0.11
...
- **Tests 2** (`hench/tests-2`): 5 files, cohesion 0.52, coupling 0.48
