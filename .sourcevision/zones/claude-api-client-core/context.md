# Claude API Client Core — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Claude API Client Core (`claude-api-client-core`)
Files: 28, Cohesion: 1.00, Coupling: 0.00
Description: Typed client, CLI provider, and auth/config helpers that encapsulate how Claude is called from local tools.
Lines: 4832
...
[observation] [info] Add integration smoke tests for cli-provider.ts and exec.ts to detect incompatible streaming or error shapes early.
[observation] [info] Centralize JSON/help-format helpers around one serialization contract to prevent CLI/API drift.
[observation] [info] Harden config/auth validation so create-client.ts cannot be instantiated without explicit key scope and rate-limits.
...
- Harden config/auth validation so create-client.ts cannot be instantiated without explicit key scope and rate-limits.
- Add integration smoke tests for cli-provider.ts and exec.ts to ensure streaming/error surfaces stay compatible with anthropic updates.
- Centralize JSON/help-format utilities around a single serialization contract to keep CLI and API providers consistent.
- Add integration smoke tests for cli-provider.ts and exec.ts to detect incompatible streaming or error shapes early.
- Centralize JSON/help-format helpers around one serialization contract to prevent CLI/API drift.
