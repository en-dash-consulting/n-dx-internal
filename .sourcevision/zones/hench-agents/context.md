...
Zone: Hench Agent Engine (`hench-agents`)
Files: 104, Cohesion: 0.99, Coupling: 0.01
Description: Large core providing agent lifecycle, analysis pipelines, schema, and tool adapters for Hench.
...
Outgoing (this zone → other zones):
  → hench-guard: packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/guard/index.ts; packages/hench/src/schema/v1.ts → packages/hench/src/guard/contracts.ts; packages/hench/src/tools/files.ts → packages/hench/src/guard/paths.ts

Incoming (other zones → this zone):
  ← hench-guard: packages/hench/tests/unit/guard/guard-integration.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/tools/shell.ts
...
[observation] [info] Cohesive agent analysis modules (adaptive/review/summary/workflow) suggest adding future heuristics by following the same interface contracts to maintain consistency.
[observation] [info] Entry points in `schema/v1.ts` and `tools/shell.ts` act as public API, so version gate any breaking changes here first.
...
- Treat `packages/hench/src/agent` as the extensibility surface—new behaviors should plug into existing lifecycle hooks.
- Stabilize the schema/tool entrypoints because other packages import them directly.
- Cohesive agent analysis modules (adaptive/review/summary/workflow) suggest adding future heuristics by following the same interface contracts to maintain consistency.
- Entry points in `schema/v1.ts` and `tools/shell.ts` act as public API, so version gate any breaking changes here first.
- [call graph] 1935 internal calls, 1 outgoing, 53 incoming (cohesion: 1, coupling: 0)
...
- **Tests** (`hench/tests`): 68 files, cohesion 0.88, coupling 0.12
...
- **Tests 3** (`hench/tests-3`): 5 files, cohesion 0.52, coupling 0.48
