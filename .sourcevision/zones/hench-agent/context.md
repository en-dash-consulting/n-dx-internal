...
Description: Complete autonomous agent package responsible for picking rex tasks, building briefs, running LLM tool-use loops, and recording run history.
Lines: 38234
...
[observation] [info] As the sole consumer of both rex and llm-client, hench's rex-gateway.ts is a high-leverage file — any upstream API changes will flow through it first, so keeping it minimal and well-tested is a priority.
[observation] [info] Cohesion 1.0 at 159 files confirms the gateway pattern is working — no cross-package import leakage detected outside the designated gateway module.
[observation] [info] The three analysis services (adaptive, review, spin) suggest hench supports multiple agent strategies; verifying they share a common interface type will keep the agent/analysis layer extensible without fragmentation.
[suggestion] [info] Zone "hench-agent" has files across 31 directories — consider consolidating under a dedicated directory
[suggestion] [warning] Add an eslint no-restricted-imports rule scoped to the hench package that disallows direct imports from 'rex' everywhere except src/prd/rex-gateway.ts. This enforces the gateway invariant mechanically rather than relying on convention.
[suggestion] [info] The hench-agent zone contains 159 files across 31 directories with no detected sub-zones, making it impossible to distinguish test files from production files in the import graph. If test files are co-resident with source files rather than under tests/, they will be silently included in production cohesion and coupling metrics.
[anti-pattern] [warning] 159-file single zone across 31 directories prevents intra-package coupling detection. Natural sub-zone boundaries exist at agent/, prd/, tools/, and cli/ subdirectories. Without sub-zone decomposition, internal tight coupling (e.g., agent analysis services depending on CLI layer details) is invisible to static analysis.
