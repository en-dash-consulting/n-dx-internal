...
Files: 141, Cohesion: 0.99, Coupling: 0.01
Risk: healthy (score: 0.01)
Description: The autonomous execution engine that picks PRD tasks, builds briefs, runs tool-use loops via the Claude API, and records run history.
Entry points: packages/hench/src/prd/llm-gateway.ts, packages/hench/src/schema/v1.ts, packages/hench/src/tools/shell.ts
Lines: 33941
...
Outgoing (this zone → other zones):
  → hench-guard: packages/hench/src/agent/lifecycle/loop.ts → packages/hench/src/guard/index.ts; packages/hench/src/tools/files.ts → packages/hench/src/guard/paths.ts

Incoming (other zones → this zone):
  ← hench-guard: packages/hench/src/guard/paths.ts → packages/hench/src/prd/llm-gateway.ts; packages/hench/tests/unit/guard/guard-integration.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/schema/v1.ts; packages/hench/tests/unit/tools/shell.test.ts → packages/hench/src/tools/shell.ts
...
[observation] [info] Cohesion 0.99 across 141 files is exceptional and reflects disciplined sub-zone structuring; the existing internal barrel index.ts files per sub-directory should be maintained as the zone grows to preserve this property.
[observation] [info] With 160+ files (per CLAUDE.md) and 31 directories, boundary assertion tests (similar to web's boundary-check.test.ts) should be added to the hench test suite before further growth to enforce sub-zone import rules programmatically.
[observation] [info] hench-guard imports 4 symbols from hench and hench imports 2 back — this bidirectional edge between the agent core and its guard layer should be audited to confirm guard policy does not depend on agent state, which would create a coupling risk as the guard is meant to be a policy enforcement layer.
[suggestion] [info] Zone "hench-agent" has files across 26 directories — consider consolidating under a dedicated directory
[pattern] [info] hench-agent (141 files, coupling 0.01) is the only large zone (100+ files) in the monorepo with near-zero outbound coupling — its gateway-mediated isolation is the strongest evidence that the gateway pattern scales to large zones and should serve as the reference model for web-viewer boundary governance.
...
- At 141 files with 0.99 cohesion, this is one of the most internally cohesive large zones in the monorepo — the sub-zone directory structure (agent/, prd/, brief/, tools/, process/) is working as intended
- The three declared entry points (llm-gateway.ts, schema/v1.ts, tools/shell.ts) expose a narrow surface compared to the zone's size, which is a strong encapsulation signal
- Low outward coupling (0.01) confirms the gateway pattern is functioning: all upstream dependencies are concentrated in llm-gateway.ts and rex-gateway.ts rather than scattered across leaf files
- Cohesion 0.99 across 141 files is exceptional and reflects disciplined sub-zone structuring; the existing internal barrel index.ts files per sub-directory should be maintained as the zone grows to preserve this property.
- hench-guard imports 4 symbols from hench and hench imports 2 back — this bidirectional edge between the agent core and its guard layer should be audited to confirm guard policy does not depend on agent state, which would create a coupling risk as the guard is meant to be a policy enforcement layer.
- With 160+ files (per CLAUDE.md) and 31 directories, boundary assertion tests (similar to web's boundary-check.test.ts) should be added to the hench test suite before further growth to enforce sub-zone import rules programmatically.
- Zone "hench-agent" has files across 26 directories — consider consolidating under a dedicated directory
- hench-agent (141 files, coupling 0.01) is the only large zone (100+ files) in the monorepo with near-zero outbound coupling — its gateway-mediated isolation is the strongest evidence that the gateway pattern scales to large zones and should serve as the reference model for web-viewer boundary governance.
- hench-agent (141 files, 0.01 coupling) achieves near-zero external coupling through the gateway pattern, but the 2 reverse imports from hench into hench-guard mean the agent zone's coupling score includes a dependency on its own policy enforcer. From a trust-boundary perspective, a guarded subject importing from its guard is equivalent to a prisoner holding a key — the guard's enforcement capability is voluntarily opt-in by the subject.
...
Cross-dependencies between sub-zones:
  hench/store → hench/unit: 6
  hench/store → hench/unit-2: 2
  hench/tools → hench/unit: 5
  hench/unit → hench/queue: 4
  hench/unit → hench/store: 2
  hench/unit → hench/tools: 6
  hench/unit → hench/unit-2: 8
  hench/unit → hench/unit-agent: 5
  hench/unit → hench/unit-cli: 3
  hench/unit → hench/unit-schema: 5
  hench/unit-2 → hench/store: 2
  hench/unit-2 → hench/tools: 1
  hench/unit-2 → hench/unit: 10
  hench/unit-agent → hench/unit: 2
  hench/unit-cli → hench/unit: 3
  hench/unit-cli → hench/unit-2: 1
  hench/unit-schema → hench/store: 1
  hench/unit-schema → hench/unit: 6
  hench/unit-schema → hench/unit-2: 1
...
- **Hench/queue** (`hench/queue`): 7 files, cohesion 0.83, coupling 0.17
- **Hench/store** (`hench/store`): 4 files, cohesion 0.29, coupling 0.71
- **Hench/tools** (`hench/tools`): 14 files, cohesion 0.79, coupling 0.21
- **Hench/unit** (`hench/unit`): 97 files, cohesion 0.9, coupling 0.1
  - Has 7 nested sub-zone(s)
- **Hench/unit 2** (`hench/unit-2`): 6 files, cohesion 1, coupling 0
