# Web Viewer Experience Shell — Zone Context

<!-- Per-zone context generated by Sourcevision. -->

<zone>

Zone: Web Viewer Experience Shell (`web-viewer-shell`)
Files: 140, Cohesion: 0.99, Coupling: 0.01
Description: Provides the front-end schemas, validators, and 130+ viewer components that render operational data such as PR markdown and task panels.
...
packages/web/src/viewer/components/sidebar.ts (TypeScript, 445 lines, source)
...
packages/web/src/viewer/main.ts (TypeScript, 377 lines, source)
...
packages/web/src/viewer/styles/pr-markdown.css (CSS, 107 lines, other)
...
packages/web/src/viewer/views/pr-markdown.ts (TypeScript, 337 lines, source)
...
packages/web/tests/unit/viewer/pr-markdown.test.ts (TypeScript, 226 lines, test)
...
Outgoing (this zone → other zones):
  → web-domain-gateways: packages/web/src/viewer/loader.ts → packages/web/src/schema/data-files.ts; packages/web/src/viewer/main.ts → packages/web/src/schema/data-files.ts

Incoming (other zones → this zone):
  ← web-domain-gateways: packages/web/tests/integration/pr-markdown-refresh.test.ts → packages/web/src/viewer/views/pr-markdown.ts
...
[observation] [info] Interactive panels lack explicit state tests, which risks UX regressions as the component surface expands.
[observation] [info] Schema validators should be versioned alongside viewer routes to prevent UI regressions when contracts evolve.
[observation] [info] Viewer components repeatedly declare shared literals; centralizing them in `components/constants.ts` would reduce duplication across 130+ files.
...
- Consolidate reusable constants/types in `viewer/components/constants.ts` and re-export them so individual components stop redefining literals.
- Version-lock schema validators in `schema/v1.ts`/`validate.ts` with view routes like `viewer/views/pr-markdown.ts` to avoid rendering mismatches when contracts change.
- Add interactive-state tests for components like `active-tasks-panel.ts` and `copy-link-button.ts` to ensure UX responsiveness as the component count grows.
- Viewer components repeatedly declare shared literals; centralizing them in `components/constants.ts` would reduce duplication across 130+ files.
- Schema validators should be versioned alongside viewer routes to prevent UI regressions when contracts evolve.
- Interactive panels lack explicit state tests, which risks UX regressions as the component surface expands.
- [call graph] 1147 internal calls, 1 outgoing, 0 incoming (cohesion: 1, coupling: 0)
...
- **Web 2** (`web-2/web-2`): 63 files, cohesion 0.91, coupling 0.09
